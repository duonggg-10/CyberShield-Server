...
@admin_endpoint.route('/api/config', methods=['GET', 'POST', 'DELETE'])
@login_required
def config_api():
    from api.utils import get_dynamic_config, _config_cache
    
    if request.method == 'GET':
        return jsonify(get_dynamic_config())
    
    if request.method == 'DELETE':
        # Refresh cache bằng cách đặt lại last_updated
        _config_cache["last_updated"] = 0
        audit_log.info(f"Config cache cleared by IP: {request.remote_addr}")
        return jsonify(success=True, message="Đã làm mới bộ nhớ đệm cấu hình!")

    if request.method == 'POST':
        return jsonify(error="Cấu hình hiện được quản lý từ xa trên GitHub. Hãy chỉnh sửa tệp trên repository để áp dụng thay đổi."), 403
